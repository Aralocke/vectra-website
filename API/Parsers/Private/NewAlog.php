...
if (!empty($Rsn))
{
    ...
    if (!is_object($Src))
    {
        ...
        elseif ($Src)
        {
            preg_match_all("#<title>([^<]+)</title>#i", $Src, $Titl) ;
            # preg_match_all("#<pubDate>\w{3}, (\d+ \w+ \d{4}) \d+:\d+:\d+ GMT</pubDate>#i", $Src, $Pubdate) ;
            preg_match_all("#<pubDate>(\w{3}, (\d+ \w+ \d{4}) \d+:\d+:\d+ GMT)</pubDate>#i", $Src, $Pubdate) ;
            preg_match_all("#<description>[\r\n\s]+([^\r\n]+)[\r\n\s]+</description>#i", $Src, $Desc) ;
            ...
            for ($Offset = 1; $Offset < count($Desc[1]); $Offset++)
            {
                if ($Titl[1][$Offset] == null)
                {
                    continue ;
                }

                $Description = trim($Desc[1][$Offset - 1]) ;
                $Description = preg_replace("/&#?[a-z0-9]{2,8};/i", "", $Description) ;
                $Description = preg_replace("#(\s{2,})#i", " ", $Description) ;
                #        htmlspecialchars($Description, ENT_NOQUOTES, 'UTF-8', true),


                $Title = trim($Titl[1][$Offset]) ;
                $Title = str_replace(
                    array('.', '&apos'), '', 
                    html_entity_decode(
                        $Title,
                ...
                else
                {
                    ...
                    if ($Type == 'all' || $Type == $Case)
                    {
                        ...
                        elseif ($Case == 'Quests')
                        {
                            $Alog[$Case][] = trim(substr($RealTitle, strpos($RealTitle, ':') + 1)) ;
                            continue ;
                        }
                        ...
                        elseif ($Case == 'Misc')
                        {
                            ...
                            elseif ($String[0] == 'Daemonheim' || $String[0] == 'Daemonheim;')
                            { 
                                if (empty($Alog[$Case]['Daemonheim']))
                                {
